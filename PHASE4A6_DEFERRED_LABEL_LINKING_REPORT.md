...
La CLI est en dry-run par défaut. L'écriture exige `--apply`. Un changement de cause ne modifie pas l'ancien lien : il le marque supersédé et crée une nouvelle version avec `supersedes_link_id`. Les causes `unknown` et `indeterminate` restent leurs propres classes et ne deviennent jamais `no_event`.
