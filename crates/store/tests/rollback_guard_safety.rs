...
async fn rollback_0013_refuses_destructively_once_a_snapshot_exists() {
    ...
    for down in [
        "0032_blue_community_evidence.down.sql",
        "0031_blue_evidence_requeue_repair.down.sql",
    ...
    for version in [13, 15, 19, 20, 21, 22, 24, 25, 26, 27, 28, 29, 30, 31, 32] {
...
/// `0032`-`0018` must roll back only in strict reverse order
/// on an empty database, and each rollback must refuse while a later
/// migration's objects still exist.
#[tokio::test]
#[allow(clippy::too_many_lines)]
async fn rollback_0032_to_0018_succeeds_only_in_reverse_order_when_empty() {
    dotenvy::dotenv().ok();
    let Ok(admin_url) = std::env::var("DATABASE_URL") else {
        eprintln!("skipping database integration test: DATABASE_URL is not configured");
        return;
    ...
    let rollback_0032 = run_psql(
        &temp_url,
        &migrations_root().join("0032_blue_community_evidence.down.sql"),
    );
    assert!(
        rollback_0032.status.success(),
        "0032 rollback must succeed before 0031: {}",
        String::from_utf8_lossy(&rollback_0032.stderr)
    );
